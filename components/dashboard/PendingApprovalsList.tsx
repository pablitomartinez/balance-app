  ...
  return (
    <div className="space-y-3">
      {approvals.map((approval) => (
        <article className="rounded-md border border-finance-line bg-white p-4" key={approval.id}>
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="font-semibold text-finance-ink">{approval.title}</p>
              <p className="mt-1 text-xs text-finance-muted">
                Solicitado por {approval.requestedByName}
              </p>
            </div>
            <p className="text-sm font-bold text-finance-warning">
